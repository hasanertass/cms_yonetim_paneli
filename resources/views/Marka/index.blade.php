                                ...
                                <tbody>
                                    @foreach($markalar as $marka)
                                        <tr>
                                            ...
                                            <form
                                                action="{{ route('marka.status', $marka->Id) }}"
                                                method="POST">
                                                @csrf
                                                <td style="width: 120px;" class="align-middle">
                                                    <button type="submit"
                                                        class="btn btn-outline-{{ $marka->status == '0' ? 'info' : 'warning' }}">
                                                        {{ $marka->status == '0' ? 'Aktif Yap' : 'Pasif Yap' }}
                                                    </button>
                                                </td>
                                            </form>
                                            <form
                                                action="{{ route('marka.home_page_status', $marka->Id) }}"
                                                method="POST">
                                                @csrf
                                                <td style="width: 120px;" class="align-middle">
                                                    <button type="submit"
                                                        class="btn btn-outline-{{ $marka->home_page_status == '0' ? 'info' : 'warning' }}">
                                                        {{ $marka->home_page_status == '0' ? 'Kullan' : 'Pasif Yap' }}
                                                    </button>
                                                </td>
                                            </form>
                                        </tr>
