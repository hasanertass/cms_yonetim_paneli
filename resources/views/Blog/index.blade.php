                                    ...
                                    @foreach($blogs as $blog)
                                        <tr>
                                            ...
                                            <td class="align-middle" style="max-width: 300px;">{!!
                                                Str::limit(strip_tags($blog->long_description), 65) !!}
                                            </td>
                                            ...
                                            @else
                                                <td style="max-width: 200px;" class="align-middle">{{ $blog->link }}
                                                </td>
                                            @endif
                                            <td class="align-middle">
                                                <form
                                            ...
                                            <form
                                                action="{{ route('blog.status', $blog->id) }}"
                                                method="POST">
                                                @csrf
                                                <td style="width: 120px;" class="align-middle">
                                                    <button type="submit"
                                                        class="btn btn-outline-{{ $blog->status == '0' ? 'info' : 'warning' }}">
                                                        {{ $blog->status == '0' ? 'Aktif Yap' : 'Pasif Yap' }}
                                                    </button>
                                                </td>
                                            </form>
                                            <form
                                                action="{{ route('blog.home_page_status', $blog->id) }}"
                                                method="POST">
                                                @csrf
                                                <td style="width: 120px;" class="align-middle">
                                                    <button type="submit"
                                                        class="btn btn-outline-{{ $blog->home_page_status == '0' ? 'info' : 'warning' }}">
                                                        {{ $blog->home_page_status == '0' ? 'Kullan' : 'Pasif Yap' }}
                                                    </button>
                                                </td>
                                            </form>
                                        </tr>
