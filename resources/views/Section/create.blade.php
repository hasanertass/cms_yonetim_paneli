    ...
    <div class="main-content position-relative max-height-vh-100 h-100">
        ...
        <div class="container-fluid py-11">
            <div class="row">
                <div class="col-md-24">
                    <div class="card">
                        <div class="card-header">Yeni Bölüm Oluştur</div>
                        @include('errors')
                        <div class="card-body">
                            <form method="POST" action="{{ route('section.store') }}"
                                ...
                                <div class="row">
                                    <div class="col-md-6">
                                        <label for="sayfa_id" class="form-label">Bölüm Yeri</label>
                                        <select class="form-control" id="sayfa_id"
                                            value="{{ old('sayfa_id') }}" name="sayfa_id" required>
                                    ...
                                    <div class="col-md-6">
                                        <label for="row">Bölüm Sırası &nbsp; <span style="color:red">Not: Bölüm sırası bilgisi
                                                sitede geçerli sayfa yüklendiğindeki listelenecek olan bölümlerin
                                                sırasını
                                                belirtir.</span></label>
                                        <input type="number" min="0" name="row" id="row"
                                            value="{{ old('row') }}" class="form-control" required>
                                    </div>
                                </div>
                                <br>
                                ...
                                <div class="form-group" id="placeholder_select" style="display: none;">
                                    <div class="row">
                                        <div class="col-md-6">
                                            ...
                                            <select class="form-control" id="placeholder" name="placeholder">
                                                ...
                                                @foreach($moduls as $modul)
                                                    <option required value="{{ $modul->modul_id }}">
                                                        {{ $modul->name }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                        <div class="col-md-6">
                                            ...
                                            <input type="number" name="data_piece" id="data_piece"
                                                value="{{ old('data_piece',1) }}" class="form-control">
                                        </div>
                                    </div>
                                </div>
                                <div class="form-group">
                                    <label for="content">Bölümün İçeriği &nbsp;  <p style="color: red; font-size: 12px; margin: 0;">Aşağıda görünen yapı sizi yanıltmasın sayfaya içerik eklediğinizde sayfada daha iyi görünebilir. :)</p></label>
                                    <textarea name="content" id="content" class="form-control"
                                        rows="20">{{ old('content') }}</textarea>
                                </div>
                                <div class="card">
                                    <button type="submit" class="btn btn-primary">Bölüm Oluştur</button>
                                </div>
                            </form>
                        </div>
                    </div>
